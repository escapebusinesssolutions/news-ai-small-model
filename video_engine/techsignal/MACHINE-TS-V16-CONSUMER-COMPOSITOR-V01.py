...
def overlay(title,beat):
 ...
 return f"drawbox=x=38:y=38:w=1004:h=235:color=0x07111e@0.72:t=fill,drawtext=fontfile={FONT}:text='{beat_t}':fontcolor=0x55d8ff:fontsize={beat_sz}:x=66:y=58,drawtext=fontfile={FONT}:text='{title_t}':fontcolor=white:fontsize={title_sz}:x=66:y=92:line_spacing={title_sp},drawtext=fontfile={FONT2}:text='TECHSIGNAL':fontcolor=0x8aa8bd:fontsize=22:x=60:y=1840"
...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='m45-v16-') as td:
  ...
  for i,s in enumerate(slots):
   seg=td/f'seg-{i:02}.mp4'; typ=s.get('asset_type'); title=s.get('title') or m.get('story',{}).get('title','TechSignal product guide'); beat=s.get('beat','technology')
