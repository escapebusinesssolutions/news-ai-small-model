...
def narrative_arc_check(m):
    required=["hook","product","key_feature","real_world_use","spec_check","tradeoff","best_for","skip_if","verdict"]
...
def main():
    ...
    for s in slots:
        ...
        if s.get("asset_type") not in {"original_graphic"} and s.get("rights_status") not in {"EDITORIAL_ALLOWED","LOCAL_TEST_ONLY","REPOSITORY_ASSET","CC_BY_3.0","CC_BY_4.0","PUBLIC_DOMAIN","CC0_1.0"}: rights_missing.append(s.get("beat"))
