...
class WordPressConfig:
    ...
    def validate(self) -> None:
        ...
        if self.publish_enabled and not self.access_token:
            raise RuntimeError("WordPress publishing is enabled but WORDPRESS_ACCESS_TOKEN is missing")
...
class WordPressPublisher:
    ...
    def authorization_check(self) -> dict[str, Any]:
        if not self.config.access_token:
            return {"ok": False, "authorized": False, "reason": "WORDPRESS_ACCESS_TOKEN missing"}

        # Validate against the configured site directly. OAuth authorization can
        # produce a single-site token; /me/sites is a global-token endpoint and
        # therefore is not a reliable gate for this publisher.
